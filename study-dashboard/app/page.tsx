              ...
              <div className="p-4 md:p-6 overflow-y-auto flex-1 bg-[#050505]/40 custom-scrollbar relative">
                <AnimatePresence mode="wait">
                  {activeTab === "questions" && (
                    ...
                    >
                      {["s1", "s2", "s3", "s5", "s6", "s7", "s8"].includes(activeSubject.id) ? (
                        <div className="pt-2">
                          <Link href={`/${activeSubject.id === "s1" ? "mis" : activeSubject.id === "s2" ? "digital-empowerment" : activeSubject.id === "s3" ? "co" : activeSubject.id === "s5" ? "ai-data-science" : activeSubject.id === "s6" ? "cpp" : activeSubject.id === "s7" ? "punjabi" : "english"}/questions`}>
                  ...
                  {activeTab === "notes" && (
                    ...
                    >
                      {["s1", "s2", "s3", "s5", "s6", "s7", "s8"].includes(activeSubject.id) ? (
                        <div className="pt-2">
                          <Link href={`/${activeSubject.id === "s1" ? "mis" : activeSubject.id === "s2" ? "digital-empowerment" : activeSubject.id === "s3" ? "co" : activeSubject.id === "s5" ? "ai-data-science" : activeSubject.id === "s6" ? "cpp" : activeSubject.id === "s7" ? "punjabi" : "english"}/notes`}>
                            <div className="border border-emerald-500/30 rounded-2xl p-6 bg-gradient-to-br from-emerald-900/20 to-transparent hover:border-emerald-400/50 hover:from-emerald-900/30 transition-all cursor-pointer group relative overflow-hidden">
                              <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/10 rounded-bl-full -z-10 group-hover:bg-emerald-500/20 transition-colors"></div>
                              <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-3">
