...
import { 
  ...
  Download, Eye, FileText, Terminal, TrendingUp, X, Filter, Lock
} from "lucide-react";
...
const subjects: Subject[] = [
  { id: "s1", name: "Management Info Systems", code: "MIS-201", progress: 0, displayDate: "April 27", examDate: "2026-04-27T09:30:00", time: "9:30 AM", color: "from-cyan-500/20 to-transparent", icon: TrendingUp, locked: false },
  { id: "s2", name: "Digital Empowerment", code: "DE-202", progress: 0, displayDate: "April 30", examDate: "2026-04-30T09:30:00", time: "9:30 AM", color: "from-blue-500/20 to-transparent", icon: BookOpen, locked: true },
  { id: "s3", name: "Computer Organization", code: "CO-203", progress: 0, displayDate: "May 02", examDate: "2026-05-02T14:00:00", time: "2:00 PM", color: "from-purple-500/20 to-transparent", icon: Terminal, locked: true },
  { id: "s4", name: "History of Punjab", code: "HOP-204", progress: 0, displayDate: "May 04", examDate: "2026-05-04T09:30:00", color: "from-amber-500/20 to-transparent", icon: FileText, locked: true },
  { id: "s5", name: "Intro to AI & Data Science", code: "AI-205", progress: 0, displayDate: "May 06", examDate: "2026-05-06T09:30:00", color: "from-cyan-400/20 to-transparent", icon: BookOpen, locked: true },
  { id: "s6", name: "OOPs using C++", code: "CPP-206", progress: 0, displayDate: "May 08", examDate: "2026-05-08T09:30:00", color: "from-emerald-500/20 to-transparent", icon: Terminal, locked: true },
  { id: "s7", name: "Punjabi", code: "PBI-207", progress: 0, displayDate: "May 11", examDate: "2026-05-11T09:30:00", color: "from-orange-500/20 to-transparent", icon: FileText, locked: true },
  { id: "s8", name: "English", code: "ENG-208", progress: 0, displayDate: "May 15", examDate: "2026-05-15T09:30:00", color: "from-slate-500/20 to-transparent", icon: BookOpen, locked: true },
];
...
export default function StudyDashboard() {
  ...
  useEffect(() => {
    ...
    const futureExams = subjects.filter(s => new Date(s.examDate).getTime() > nowTime);
    futureExams.sort((a, b) => new Date(a.examDate).getTime() - new Date(b.examDate).getTime());
    ...
    if (futureExams.length > 0) {
      ...
      const interval = setInterval(() => {
        ...
        if (distance < 0) {
          ...
          // Simple reload or state update logic could go here when an exam passes
        } else {
          setTimeLeft({
    ...
    <div className="min-h-screen bg-[#050505] text-[#f8fafc] font-sans selection:bg-cyan-500/30 overflow-hidden relative pb-20 md:pb-0">
            ...
            {nextExam && (
              <div className="bg-black/50 border border-white/5 rounded-2xl p-4 flex flex-col sm:flex-row gap-4 items-center backdrop-blur-md w-full xl:w-auto">
                <div className="text-xs text-slate-400 font-medium tracking-wide sm:border-r border-white/10 sm:pr-4 uppercase">
                  Next Exam:<br/>
                  <span className="text-white normal-case text-sm">{nextExam.name}</span>
                </div>
                <div className="flex gap-4">
                  <div className="text-center">
                    <div className="text-xl md:text-2xl font-mono font-bold" style={{ color: accentColor }}>{timeLeft.days.toString().padStart(2, '0')}</div>
                    <div className="text-[10px] uppercase tracking-wider text-slate-500">Days</div>
                  </div>
                  <div className="text-center">
                    <div className="text-xl md:text-2xl font-mono font-bold">{timeLeft.hours.toString().padStart(2, '0')}</div>
                    <div className="text-[10px] uppercase tracking-wider text-slate-500">Hrs</div>
                  </div>
                  <div className="text-center">
                    <div className="text-xl md:text-2xl font-mono font-bold">{timeLeft.mins.toString().padStart(2, '0')}</div>
                    <div className="text-[10px] uppercase tracking-wider text-slate-500">Min</div>
                  </div>
                  <div className="text-center">
                    <div className="text-xl md:text-2xl font-mono font-bold">{timeLeft.secs.toString().padStart(2, '0')}</div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {subjects.map((subject, idx) => (
                <motion.div
                  ...
                  className={`glass-panel rounded-2xl p-5 relative overflow-hidden group border border-white/5 transition-all duration-300 ${subject.locked ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer hover:border-white/20'}`}
                >
              ...
              <div className="space-y-6 relative before:absolute before:inset-0 before:ml-[11px] before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-white/10 before:to-transparent">
                {upcomingExams.map((sub, i) => (
                  <div key={i} className="relative flex items-center group is-active">
                    {/* Timeline Dot */}
                    <div className="flex items-center justify-center w-6 h-6 rounded-full border border-white/10 bg-[#050505] shadow shrink-0 z-10 transition-colors duration-300" style={{ boxShadow: i === 0 ? `0 0 10px ${accentGlow}` : 'none', borderColor: i === 0 ? accentColor : 'rgba(255,255,255,0.1)' }}>
                    ...
                    <div className="w-[calc(100%-2rem)] glass-panel border border-white/5 p-3 rounded-xl ml-4 shadow-sm group-hover:border-white/10 transition-colors">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-1 gap-1">
                        <span className="text-[10px] font-mono text-slate-400">{sub.displayDate}</span>
                        {sub.time && <span className="text-[9px] w-max px-1.5 py-0.5 bg-white/5 rounded text-slate-300">{sub.time}</span>}
                      </div>
      ...
      <AnimatePresence>
        {activeSubject && (
          ...
          >
            <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setActiveSubject(null)} />
                ...
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-[10px] md:text-xs text-slate-400 flex items-center gap-1">
                      <Clock className="w-3 h-3" /> {activeSubject.displayDate} {activeSubject.time && `• ${activeSubject.time}`}
                    </span>
                  </div>
                  <h2 className="text-xl md:text-2xl font-semibold text-white">{activeSubject.name}</h2>
                </div>
                <button 
              ...
              <div className="flex overflow-x-auto border-b border-white/5 px-2 md:px-6 hide-scrollbar shrink-0">
                {[
                  ...
                  <button
                    ...
                    className={`px-4 py-4 text-xs md:text-sm font-medium transition-colors relative whitespace-nowrap ${activeTab === tab.id ? 'text-white' : 'text-slate-500 hover:text-slate-300'}`}
                  >
                    {tab.label}
                    {activeTab === tab.id && (
                      <motion.div 
                  ...
                  {activeTab === 'questions' && (
                    ...
                    >
                      <div className="text-center py-12 border border-dashed border-white/10 rounded-2xl bg-white/[0.01]">
                        ...
                        <h3 className="text-slate-300 font-medium">No questions added yet</h3>
                        <p className="text-sm text-slate-500 mt-1">Provide the material to populate the important questions for {activeSubject.code}.</p>
                      </div>
                    </motion.div>
                  )}
                  ...
                  {activeTab === 'notes' && (
                    ...
                    >
                      <div className="col-span-full text-center py-12 border border-dashed border-white/10 rounded-2xl bg-white/[0.01]">
                        ...
                        <h3 className="text-slate-300 font-medium">Notes vault empty</h3>
                        <p className="text-sm text-slate-500 mt-1">Awaiting PDF uploads and module materials.</p>
                      </div>
                    </motion.div>
                  )}
                  ...
                  {activeTab === 'units' && (
                    ...
                    >
                      <div className="text-center py-12 border border-dashed border-white/10 rounded-2xl bg-white/[0.01]">
                        ...
                        <h3 className="text-slate-300 font-medium">Syllabus missing</h3>
                        <p className="text-sm text-slate-500 mt-1">Provide the unit-wise syllabus to generate study tracks.</p>
                      </div>
                    </motion.div>
                  )}
