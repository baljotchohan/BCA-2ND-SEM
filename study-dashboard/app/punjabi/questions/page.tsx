...
export default function PunjabiQuestionsPage() {
  return (
        ...
        {questionsData.map((unit, idx) => (
          <section key={idx} className="mb-16 print:break-after-page">
            ...
            {unit.questions.map((q, qIdx) => (
              <QuestionBlock key={qIdx} qId={q.id} title={q.title} marks={q.marks}>
                <Solve pa={q.pa} />
              </QuestionBlock>
            ))}
          </section>
...
function QuestionBlock({ qId, title, marks, children }: { qId: string, title: string, marks?: string, children: React.ReactNode }) {
  return (
    <div className="mb-10 print:break-inside-avoid">
      <div className="flex items-start gap-4 mb-5">
        <div className="bg-[#ea580c] text-white px-3 py-1.5 rounded-lg font-black text-sm shadow-sm shrink-0 mt-1">
          {qId}
        </div>
        <div className="flex-1">
          <div className="flex items-center justify-between">
            ...
            {marks && (
              <span className="text-sm font-bold text-orange-600 bg-orange-100 px-2 py-1 rounded ml-4 shrink-0">
                {marks} Marks
              </span>
            )}
          </div>
...
function FormattedContent({ text }: { text: string }) {
  ...
  const parseLine = (line: string) => {
    return line
      ...
      .replace(/`([^`]+)`/g, '<code class="bg-slate-100 px-1 py-0.5 rounded text-orange-600 font-mono text-xs">$1</code>');
  };

  const segments = text.split('\`\`\`');
  
  return (
    <div className="space-y-3 text-base text-slate-800 leading-relaxed font-medium">
      {segments.map((segment, index) => {
        if (index % 2 === 1) {
          const firstLineBreak = segment.indexOf('\n');
          const codeContent = firstLineBreak !== -1 ? segment.substring(firstLineBreak + 1) : segment;
          return (
            <pre key={index} className="bg-slate-900 text-orange-400 p-4 rounded-lg font-mono text-sm overflow-x-auto my-4 whitespace-pre">
              {codeContent}
            </pre>
          );
        }
...
function Solve({ pa }: { pa: string }) {
  return (
    <div>
      <h4 className="text-[#ea580c] font-bold mb-4 flex items-center gap-2 border-b border-slate-100 pb-2 text-sm uppercase tracking-wider">
        ਉੱਤਰ (Solution)
      </h4>
